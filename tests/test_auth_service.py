...
class TestAuthRequired(unittest.TestCase):
    ...
    def test_unset_non_prod_without_pytest_follows_secret(self):
        env = os.environ.copy()
        env.pop("AUTH_REQUIRED", None)
        env.pop("PYTEST_CURRENT_TEST", None)
        env.pop("AUTH_TOKEN_SECRET", None)
    ...
    def test_pytest_auto_off_even_if_app_env_production(self):
        env = os.environ.copy()
        env.pop("AUTH_REQUIRED", None)
        env["PYTEST_CURRENT_TEST"] = "yes"
        env["APP_ENV"] = "production"
        ...
        with patch.dict(os.environ, env, clear=True):
            self.assertFalse(auth_required())
            set_current_principal(None)
            self.assertIsNone(require_principal(roles=["teacher"]))
