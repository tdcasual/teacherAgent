...
import {
  selectArchiveDialogMeta,
  selectGroupedSessions,
  selectVisibleSessions,
} from '../features/chat/studentUiSelectors';
...
export function useSessionManager({
  ...
  const { historyRequestRef, sessionRequestRef, pendingChatJobRef, recentCompletedRepliesRef } =
    refs;
    ...
    async (mode: 'reset' | 'more' = 'reset') => {
      ...
      try {
        ...
        const serverIds = new Set(
          serverSessions.map((item) => String(item.session_id || '').trim()).filter(Boolean),
        );
        dispatch({
          type: 'UPDATE_LOCAL_DRAFT_SESSION_IDS',
          updater: (prev) => prev.filter((id) => !serverIds.has(id)),
        });
          ...
          pending?.job_id && pending.session_id === targetSessionId
            ? (() => {
                ...
                const hasUserText = pending.user_text
                  ? base.some((item) => item.role === 'user' && item.content === pending.user_text)
                  : true;
        ...
        const merged: Message[] =
          recentReplies.length && !(pending?.job_id && pending.session_id === targetSessionId)
            ? (() => {
                ...
                for (const recent of [...recentReplies].sort(
                  ...
                  let matchedIndex = assistantHistoryCandidates.findIndex(
                    (c) =>
                      !c.used &&
                      c.content === recent.reply_text &&
                      Number.isFinite(c.ts_ms) &&
                      c.ts_ms >= recent.completed_at - RECENT_COMPLETION_TTL_MS,
                  );
                  if (
                    matchedIndex < 0 &&
                    recent.user_text &&
                    (userHistoryCounts[recent.user_text] || 0) > 0
                  ) {
                    matchedIndex = assistantHistoryCandidates.findIndex(
                      (c) =>
                        !c.used && c.content === recent.reply_text && !Number.isFinite(c.ts_ms),
                    );
                  }
                  if (matchedIndex >= 0) {
                ...
                for (const recent of unresolvedRecent) {
                  if (recent.user_text)
                    patched.push({
                      id: makeId(),
                      role: 'user',
                      content: recent.user_text,
                      time: nowTime(),
                    });
                  patched.push({
                    id: makeId(),
                    role: 'assistant',
                    content: recent.reply_text,
                    time: nowTime(),
                  });
                }
                if (removableKeys.size > 0) {
                  dispatch({
                    ...
                    updater: (prev) =>
                      prev.filter((item) => !removableKeys.has(recentCompletionKeyOf(item))),
                  });
                }
                return patched;
              })()
  ...
  const selectStudentSession = useCallback(
    (sessionId: string) => {
      ...
      dispatch({
        type: 'SET',
        field: 'forceSessionLoadToken',
        value: state.forceSessionLoadToken + 1,
      });
  ...
  const startNewStudentSession = useCallback(() => {
    ...
    dispatch({
      type: 'UPDATE_LOCAL_DRAFT_SESSION_IDS',
      updater: (prev) => (prev.includes(next) ? prev : [next, ...prev]),
    });
    dispatch({
      type: 'BATCH',
      actions: [
    ...
    dispatch({
      ...
      updater: (prev) => {
        ...
        return [
          { session_id: next, updated_at: new Date().toISOString(), message_count: 0, preview: '' },
          ...prev,
        ];
      },
    });
  ...
  const confirmArchiveDialog = useCallback(() => {
    ...
    dispatch({
      type: 'UPDATE_DELETED_SESSION_IDS',
      updater: (prev) => {
