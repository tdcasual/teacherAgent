...
describe('teacher mobile shell state mapping', () => {
  it('maps tab to panel state', () => {
    expect(teacherMobilePanelsFromTab('chat')).toEqual({
      sessionSidebarOpen: false,
      skillsOpen: false,
    });
    expect(teacherMobilePanelsFromTab('sessions')).toEqual({
      sessionSidebarOpen: true,
      skillsOpen: false,
    });
    expect(teacherMobilePanelsFromTab('workbench')).toEqual({
      sessionSidebarOpen: false,
      skillsOpen: true,
    });
  });
  ...
  it('maps panel state to tab', () => {
    expect(teacherMobileTabFromPanels({ sessionSidebarOpen: false, skillsOpen: false })).toBe(
      'chat',
    );
    expect(teacherMobileTabFromPanels({ sessionSidebarOpen: true, skillsOpen: false })).toBe(
      'sessions',
    );
    expect(teacherMobileTabFromPanels({ sessionSidebarOpen: false, skillsOpen: true })).toBe(
      'workbench',
    );
    expect(teacherMobileTabFromPanels({ sessionSidebarOpen: true, skillsOpen: true })).toBe(
      'workbench',
    );
  });
