...
def _proposals_dir(teacher_id: str, *, deps: StudentMemoryDeps) -> Path:
    base = deps.teacher_workspace_dir(teacher_id) / "student_memory" / "proposals"
    base.mkdir(parents=True, exist_ok=True)
    return base


def _proposal_path(teacher_id: str, proposal_id: str, *, deps: StudentMemoryDeps) -> Path:
    return _proposals_dir(teacher_id, deps=deps) / f"{_safe_fs_id(proposal_id, prefix='smem')}.json"


def _event_log_path(teacher_id: str, *, deps: StudentMemoryDeps) -> Path:
