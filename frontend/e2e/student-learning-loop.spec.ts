...
test('student shell defaults to today-first home and enters chat on primary action', async ({ page }) => {
  ...
  await expect(page.getByText('历史与补充')).toHaveCount(0)
  await expect(page.getByTestId('student-home-history-stage')).toHaveAttribute('data-home-tier', 'supporting')
  await expect(page.getByTestId('student-home-history-stage')).toHaveAttribute('data-home-style', 'inline-links')
