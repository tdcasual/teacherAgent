...
from services.api.session_history_service import (
    SessionHistoryDeps,
    SessionHistoryError,
...
def _make_deps() -> tuple[SessionHistoryDeps, Dict[str, Any]]:
    state: Dict[str, Any] = {
        "student_states": {
...
def test_student_history_sessions_success_and_validation() -> None:
    ...
    with pytest.raises(SessionHistoryError, match="student_id is required"):
...
def test_student_history_session_success_and_validation() -> None:
    ...
    with pytest.raises(SessionHistoryError, match="student_id and session_id are required"):
