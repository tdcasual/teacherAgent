...
export default function UploadSection(props: Props) {
  ...
  useEffect(() => {
    ...
    void fetch(`${apiBase}/teacher/roster`, {
      ...
      .then(async (res) =>
        res.ok ? ((await res.json()) as { items?: RosterItem[] }) : { items: [] },
      )
      .then((payload) => {
  ...
  const subjectOptions = useMemo(() => {
    ...
    const ids = rosterSubjects.length
      ? rosterSubjects
      : ASSIGNMENT_SUBJECT_OPTIONS.map((option) => option.id);
  ...
  return (
    <section
      id="workflow-upload-section"
      className={`bg-surface border border-border rounded-[14px] shadow-sm ${uploadCardCollapsed ? 'py-[10px] px-3' : 'p-[10px]'}`}
    >
      <div
        className={`panel-header flex items-start gap-2 flex-wrap ${uploadCardCollapsed ? 'mb-0' : 'mb-2'}`}
      >
        <div className="flex items-center gap-3 min-w-0 flex-1">
          <h3 className="m-0 whitespace-nowrap shrink-0">上传作业文件（文档 / 图片）</h3>
        </div>
        {uploadCardCollapsed ? (
          <div
            className="panel-summary flex-1 min-w-0 text-muted text-[12px] whitespace-nowrap overflow-hidden text-ellipsis"
            title={formatUploadJobSummary(uploadJobInfo, uploadAssignmentId.trim())}
          >
      ...
      {uploadCardCollapsed ? null : (
        <>
          <p className="m-0 mb-3 text-muted">
            上传后将在后台解析题目与答案，并生成作业 8 点描述。解析完成后需确认创建作业。
          </p>
            ...
            <div className="grid gap-[10px] grid-cols-1">
              ...
              <LabeledField label="截止日期（可选）">
                <input
                  ...
                  placeholder="YYYY-MM-DD 或 ISO 时间"
                />
              </LabeledField>
              ...
              <LabeledField label="范围">
                <select
                  value={uploadScope}
                  onChange={(e) => setUploadScope(e.target.value as UploadScope)}
                >
