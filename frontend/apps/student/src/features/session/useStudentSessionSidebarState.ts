...
export function useStudentSessionSidebarState({
  ...
  openSessionMenuId,
  dispatch,
}: UseStudentSessionSidebarStateParams) {
  ...
  const toggleSessionMenu = useCallback(
    (sessionId: string) => {
      ...
      dispatch({
        type: 'SET',
        field: 'openSessionMenuId',
        value: openSessionMenuId === sid ? '' : sid,
      });
    },
  ...
  const setSessionMenuTriggerRef = useCallback(
  ...
  const focusSessionMenuItem = useCallback((sessionId: string, target: 'first' | 'last') => {
    ...
    const items = Array.from(
      menu.querySelectorAll<HTMLButtonElement>('.session-menu-item:not([disabled])'),
    );
