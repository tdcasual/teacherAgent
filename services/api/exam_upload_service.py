...
class ExamUploadError(Exception):
...
def _load_job_or_raise(job_id: str, *, deps: ExamUploadDeps) -> Dict[str, Any]:
    try:
        job = deps.load_exam_job(job_id)
    except FileNotFoundError as exc:
        raise ExamUploadError(status_code=404, detail="job not found") from exc
    try:
        enforce_upload_job_access(job)
    except AuthError as exc:
        raise ExamUploadError(status_code=exc.status_code, detail=exc.detail) from exc
    return job


def exam_upload_status(job_id: str, *, deps: ExamUploadDeps) -> Dict[str, Any]:
    return _load_job_or_raise(job_id, deps=deps)


def exam_upload_draft(job_id: str, *, deps: ExamUploadDeps) -> Dict[str, Any]:
    job = _load_job_or_raise(job_id, deps=deps)
    status = job.get("status")
    if status not in {"done", "confirmed"}:
        raise ExamUploadError(
            status_code=400,
    ...
    if not parsed_path.exists():
        raise ExamUploadError(status_code=400, detail="parsed result missing")
...
def exam_upload_confirm(job_id: str, *, deps: ExamUploadDeps) -> Dict[str, Any]:
    job = _load_job_or_raise(job_id, deps=deps)
    status = job.get("status")
    if status == "confirmed":
    ...
    try:
        ...
        if status_code is not None:
            raise ExamUploadError(status_code=status_code, detail=getattr(exc, "detail", str(exc))) from exc
        raise
