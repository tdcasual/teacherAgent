...
export type StudentAction =
  ...
  | {
      ...
      updater: (prev: RecentCompletedReply[]) => RecentCompletedReply[];
    }
...
function buildInitialState(): StudentState {
  ...
  if (rawVerified) {
    try {
      verifiedStudent = JSON.parse(rawVerified) as VerifiedStudent;
    } catch {
      /* ignore */
    }
  }
  ...
  return {
    ...
    messages: [
      { id: makeId(), role: 'assistant', content: STUDENT_WELCOME_MESSAGE, time: nowTime() },
    ],
    input: '',
    sending: false,
    pendingChatJob,
...
export function useStudentState() {
  ...
  const markdownCacheRef = useRef(
    new Map<string, { content: string; html: string; apiBase: string }>(),
  );
