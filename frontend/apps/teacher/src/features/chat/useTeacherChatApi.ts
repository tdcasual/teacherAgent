...
export function useTeacherChatApi(params: UseTeacherChatApiParams) {
  ...
  const markdownCacheRef = useRef(
    new Map<string, { content: string; html: string; apiBase: string; authToken: string }>(),
  );
  ...
  const syncHistoryCursor = useCallback((val: number) => {
    historyCursorRef.current = val;
  }, []);
  const syncHistoryHasMore = useCallback((val: boolean) => {
    historyHasMoreRef.current = val;
  }, []);
  const syncLocalDraftSessionIds = useCallback((val: string[]) => {
    localDraftSessionIdsRef.current = val;
  }, []);
  ...
  const renderedMessages = useMemo(() => {
    ...
    const pendingPlaceholderId = pendingChatJob?.job_id
      ? String(pendingChatJob.placeholder_id || '').trim()
      : '';
  ...
  const loadTeacherSessionMessages = useCallback(
    async (sessionId: string, cursor: number, append: boolean) => {
      ...
      try {
        ...
        if (requestNo !== sessionRequestRef.current || activeSessionRef.current !== targetSessionId)
          return;
        ...
        if (append) {
          ...
          setMessages((prev) => {
            ...
            if (
              pendingBelongsToTargetSession &&
              prev.some((item) => String(item.id || '').startsWith('pending_user_'))
            ) {
              return prev;
            }
            return [
  ...
  const reviewStudentMemoryProposal = useCallback(
    async (proposalId: string, approve: boolean) => {
      ...
      const res = await fetch(
        `${apiBase}/teacher/student-memory/proposals/${encodeURIComponent(pid)}/review`,
        {
          method: 'POST',
  ...
  const deleteStudentMemoryProposal = useCallback(
    async (proposalId: string) => {
      ...
      if (!pid) throw new Error('proposal_id 缺失');
      const res = await fetch(
        `${apiBase}/teacher/student-memory/proposals/${encodeURIComponent(pid)}`,
        {
          method: 'DELETE',
        },
      );
  ...
  const postToolConfirm = useCallback(
  ...
  const confirmToolWrite = useCallback(() => {
    void postToolConfirm(true);
  }, [postToolConfirm]);
  const cancelToolConfirm = useCallback(() => {
    void postToolConfirm(false);
  }, [postToolConfirm]);
  // ── Return ────────────────────────────────────────────────────────────
  return {
    refreshTeacherSessions,
