...
def test_auto_route_rules_hotspot_removed() -> None:
    ...
    assert "def _score_lesson_capture(" not in source
    assert "def _score_core_examples(" not in source
    assert "def _score_student_focus(" not in source
    ...
    assert "physics-lesson-capture" not in source
    assert "physics-core-examples" not in source
    assert "physics-student-focus" not in source
