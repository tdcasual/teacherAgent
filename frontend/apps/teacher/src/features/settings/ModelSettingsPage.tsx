...
export default function ModelSettingsPage({ apiBase, onApiBaseChange }: Props) {
  ...
  const handleModeChange = useCallback(
    (purpose: PurposeKey, mode: string) => {
      ...
      updatePurposeModel(purpose, {
        mode,
        model: selected?.default_model || models[purpose].model,
      });
    },
    ...
    async (providerId: string) => {
      ...
      try {
        const res = await fetch(
          `${apiBase}/teacher/provider-registry/providers/${encodeURIComponent(id)}`,
          {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({}),
        ...
        <div className="grid gap-3">
          {PURPOSES.map((purpose) => {
            ...
            return (
              <div
                key={purpose.key}
                className="rounded-lg border border-border bg-surface-soft p-3 grid gap-2"
              >
                ...
                <div className="grid gap-2 md:grid-cols-3">
                  ...
                  <label className="grid gap-1" htmlFor={`${formId}-${purpose.key}-mode`}>
                    ...
                    <select
                      id={`${formId}-${purpose.key}-mode`}
                      value={current.mode}
                      onChange={(event) => handleModeChange(purpose.key, event.target.value)}
                    >
                      <option value="">请选择</option>
                      {modes.map((mode) => (
                  ...
                  <label className="grid gap-1" htmlFor={`${formId}-${purpose.key}-model`}>
                    ...
                    <input
                      ...
                      value={current.model}
                      onChange={(event) =>
                        updatePurposeModel(purpose.key, { model: event.target.value })
                      }
        ...
        <div className="grid gap-2 md:grid-cols-2">
          <label className="grid gap-1" htmlFor={`${formId}-provider-id`}>
            ...
            <input
              ...
              placeholder="例如 tprv_openrouter_01"
            />
          </label>
          ...
          <label className="grid gap-1" htmlFor={`${formId}-provider-default-model`}>
            ...
            <input
              id={`${formId}-provider-default-model`}
              value={newProviderDefaultModel}
              onChange={(event) => setNewProviderDefaultModel(event.target.value)}
              ...
              return (
                <div key={providerId} className="rounded-lg border border-border p-2 grid gap-1">
                  ...
                  <div className="text-[12px] text-muted">
                    key: {provider.api_key_masked || '***'} | 默认模型:{' '}
                    {provider.default_model || '-'}
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-[12px] text-muted">
                      {provider.enabled === false ? '状态：停用' : '状态：启用'}
                    </span>
