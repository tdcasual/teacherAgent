...
export type UseTeacherChatStreamParams = {
  ...
  setToolConfirm: Dispatch<SetStateAction<{ confirm_id: string; tool: string; preview: string } | null>>
}
...
export function useTeacherChatStream(params: UseTeacherChatStreamParams) {
  ...
  useEffect(() => {
    ...
    const { setPlaceholderContent, finishSuccess, finishFailure } = createTeacherChatPendingJobHandlers({
      pendingChatJob,
      pendingChatJobRef,
      activeSessionId,
      setMessages,
    ...
    const runStream = async () => {
      ...
      while (!stopped) {
        ...
        try {
          ...
          const statusData = await fetchTeacherChatJobStatus(apiBase, pendingChatJob.job_id, controller.signal)
  ...
  }, [pendingChatJob, pendingChatJob?.job_id, apiBase, authToken, refreshTeacherSessions, activeSessionId, setMessages, setPendingChatJob, setChatQueueHint, setPendingStreamStage, setPendingToolRuns, setExecutionTimeline, setSending, setComposerWarning, skillList, pendingChatJobRef, setToolConfirm])
}
