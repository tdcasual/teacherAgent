...
def adapt_pdf_report_summary(payload: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    lines = split_nonempty_lines(normalized_text)
    ...
    title = str(payload_final.get('title') or 'PDF 摘要报告').strip() or 'PDF 摘要报告'
    ...
    bundle = ClassSignalBundle(
        source_meta=ClassSignalSourceMeta(
            title=title,
            provider='pdf_summary',
            source_type='pdf_report_summary',
            report_id=report_id,
        ),
        ...
        attachments=_source_attachment(context_final, payload_final, default_name='class-report.pdf', mime_type='application/pdf'),
        parse_confidence=float(payload_final.get('parse_confidence') or 0.68),
