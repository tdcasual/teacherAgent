...
export default function SessionSidebar(props: Props) {
  const {
    apiBase, sidebarOpen, dispatch, verifiedStudent, historyLoading, historyError,
    historyHasMore, refreshSessions, showArchivedSessions, historyQuery,
    visibleSessionCount, groupedSessions, deletedSessionIds, activeSessionId,
    onSelectSession, getSessionTitle, openSessionMenuId, toggleSessionMenu,
    ...
    sessionCursor, loadSessionMessages, sessionError, verifyOpen, handleVerify,
    nameInput, classInput, verifying, verifyError, todayAssignment, assignmentLoading,
    assignmentError, resetVerification, startNewStudentSession, renameDialogSessionId,
    archiveDialogSessionId, archiveDialogActionLabel, archiveDialogIsArchived,
    cancelRenameDialog, confirmRenameDialog, cancelArchiveDialog, confirmArchiveDialog,
  } = props
  ...
  return (
    <>
      <button
        ...
        aria-label="关闭会话侧栏"
        onClick={() => dispatch({ type: 'SET', field: 'sidebarOpen', value: false })}
      />
      <aside className={`session-sidebar ${sidebarOpen ? 'open' : 'collapsed'}`}>
        ...
        <section className="border-t border-border pt-2.5 grid gap-2">
          <div className="flex justify-between items-center gap-2">
            <strong>学习信息</strong>
            <button type="button" className="ghost" onClick={() => dispatch({ type: 'SET', field: 'verifyOpen', value: !verifyOpen })}>
              {verifyOpen ? '收起' : '展开'}
            </button>
          </div>
          {verifiedStudent ? (
          ...
          {verifyOpen && (
            <form className="grid gap-2.5" onSubmit={handleVerify}>
              <div className="grid gap-1.5">
                <label>姓名</label>
                <input value={nameInput} onChange={(e) => dispatch({ type: 'SET', field: 'nameInput', value: e.target.value })} placeholder="例如：刘昊然" />
              </div>
              <div className="grid gap-1.5">
                <label>班级（重名时必填）</label>
                <input value={classInput} onChange={(e) => dispatch({ type: 'SET', field: 'classInput', value: e.target.value })} placeholder="例如：高二2403班" />
              </div>
