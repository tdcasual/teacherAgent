  ...
  return (
    <div className="grid gap-3 rounded-2xl border border-border p-4">
      ...
      <div className="text-xs text-muted">
        导入名册后先加任教，再整班入学。不要指望 CSV 自动编班。
      </div>
      <div className="grid gap-3 md:grid-cols-3">
        <div className="grid gap-1">
          <label className="text-xs text-muted" htmlFor={teacherId}>
            教师
          </label>
          <select
            id={teacherId}
        ...
        <div className="grid gap-1">
          <label className="text-xs text-muted" htmlFor={subjectId}>
            学科
          </label>
          <select
            id={subjectId}
        ...
        <div className="grid gap-1">
          <label className="text-xs text-muted" htmlFor={classId}>
            班级
          </label>
          <input
            id={classId}
      ...
      <div className="flex flex-wrap gap-2">
        <button type="button" className="ghost" onClick={() => void handleRoster()} disabled={busy}>
          加任教
        </button>
        <button
          type="button"
          className="ghost"
          onClick={() => void handleEnrollClass()}
          disabled={busy}
        >
          整班入学
        </button>
            ...
            return (
              ...
              >
                ...
                <button
                  type="button"
                  className="ghost"
                  onClick={() => void handleClaim(assignmentId)}
                  disabled={busy}
                >
                  认领给当前教师
                </button>
