...
def enqueue_upload_job_inline(job_id: str, *, deps: UploadWorkerDeps) -> bool:
    enqueued = False
    with deps.job_lock:
...
def scan_pending_upload_jobs_inline(*, deps: UploadWorkerDeps) -> int:
    ...
    for job_path in deps.job_dir.glob("*/job.json"):
        ...
        if status in {"queued", "processing"} and job_id:
            if enqueue_upload_job_inline(job_id, deps=deps):
                count += 1
    return count
...
def stop_upload_worker(*, deps: UploadWorkerDeps, timeout_sec: float = 1.5) -> None:
    ...
    if thread is not None:
        try:
            ...
            _log.debug("upload worker thread join failed", exc_info=True)
        is_alive = False
        try:
            is_alive_method = getattr(thread, "is_alive", None)
