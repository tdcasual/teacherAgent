...
class StudentMemoryAutoTest(unittest.TestCase):
    def test_student_turn_auto_creates_controlled_proposal(self):
        with TemporaryDirectory() as td:
            ...
            app_mod.get_core().run_agent = lambda *args, **kwargs: {"reply": "收到，后续我会按你说的方式回答。"}  # type: ignore[attr-defined]
    ...
    def test_student_turn_auto_blocked_content_records_event_without_proposal(self):
        with TemporaryDirectory() as td:
            ...
            job_saved = app_mod.get_core().load_chat_job(record["job_id"])
            ...
            log_path = app_mod.get_core().teacher_workspace_dir(teacher_id) / "telemetry" / "student_memory_events.jsonl"
