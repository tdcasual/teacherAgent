...
class ClassReportServiceError(Exception):
...
def write_class_report_job(job_id: str, payload: Dict[str, Any], *, deps: ClassReportDeps) -> Dict[str, Any]:
    target = _job_relative_path(job_id)
...
def load_class_report_job(job_id: str, *, deps: ClassReportDeps) -> Dict[str, Any]:
    return deps.metadata_repo.read_json(_job_relative_path(job_id))



def write_class_signal_bundle(job_id: str, payload: Dict[str, Any], *, deps: ClassReportDeps) -> Dict[str, Any]:
    deps.metadata_repo.write_json(_bundle_relative_path(job_id), dict(payload or {}))
    return dict(payload or {})



def load_class_signal_bundle(job_id: str, *, deps: ClassReportDeps) -> Dict[str, Any]:
    return deps.metadata_repo.read_json(_bundle_relative_path(job_id))



def write_class_report(report_id: str, payload: Dict[str, Any], *, deps: ClassReportDeps) -> Dict[str, Any]:
    deps.metadata_repo.write_json(_report_relative_path(report_id), dict(payload or {}))
    return dict(payload or {})



def load_class_report(report_id: str, *, deps: ClassReportDeps) -> Dict[str, Any]:
    return deps.metadata_repo.read_json(_report_relative_path(report_id))



def deliver_class_report(
    *,
    job: Dict[str, Any],
    bundle: Dict[str, Any],
    ...
    return write_class_report(report_id, payload, deps=deps)



def enqueue_class_report_review_item(
    *,
    report_id: str,
    teacher_id: str,
    reason: str,
...
def list_class_report_review_queue(*, teacher_id: str, deps: ClassReportDeps) -> Dict[str, Any]:
    teacher_id_final = _require_teacher_id(teacher_id)
    return list_review_items(teacher_id=teacher_id_final, domain='class_report', status=None, deps=deps.review_queue_deps)



def list_class_reports(*, teacher_id: str, status: str | None, deps: ClassReportDeps) -> Dict[str, Any]:
    teacher_id_final = _require_teacher_id(teacher_id)
...
def get_class_report(report_id: str, *, teacher_id: str, deps: ClassReportDeps) -> Dict[str, Any]:
    teacher_id_final = _require_teacher_id(teacher_id)
    ...
    bundle = {}
    job_id = str(job.get('job_id') or summary['report_id'] or '').strip()
    if job_id:
        bundle = _read_optional_json(_bundle_relative_path(job_id), deps) or {}
    return {
        'report': summary,
        'analysis_artifact': {},
        'artifact_meta': {
...
def rerun_class_report(report_id: str, *, teacher_id: str, reason: str | None, deps: ClassReportDeps) -> Dict[str, Any]:
    teacher_id_final = _require_teacher_id(teacher_id)
    report, job = _load_report_or_job(report_id, deps)
...
def _load_report_or_job(report_id: str, deps: ClassReportDeps) -> Tuple[Optional[Dict[str, Any]], Optional[Dict[str, Any]]]:
    ...
    if report is not None:
        ...
        job = _read_optional_json(_job_relative_path(job_id), deps) if job_id else None
        return report, job
...
def _summary_from_report(report: Dict[str, Any]) -> Dict[str, Any]:
    report_id = str(report.get('report_id') or report.get('job_id') or '').strip()
    return {
        'report_id': report_id,
        'teacher_id': str(report.get('teacher_id') or '').strip(),
        'analysis_type': 'class_report',
        'target_type': str(report.get('target_type') or 'report').strip() or 'report',
        'target_id': str(report.get('target_id') or report_id).strip() or report_id,
        'strategy_id': str(report.get('strategy_id') or 'class_signal.teacher.report').strip() or 'class_signal.teacher.report',
...
def _read_optional_json(relative_path: str, deps: ClassReportDeps) -> Optional[Dict[str, Any]]:
    try:
...
def _job_relative_path(job_id: str | None) -> str:
    return f'jobs/{str(job_id or "").strip()}.json'



def _bundle_relative_path(job_id: str | None) -> str:
    return f'bundles/{str(job_id or "").strip()}.json'
...
def _require_teacher_id(teacher_id: str) -> str:
    ...
    if not teacher_id_final:
        raise ClassReportServiceError(400, 'teacher_id_required')
