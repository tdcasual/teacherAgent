...
export default function AssignmentDraftSection(props: Props) {
  ...
  return (
    <section
      id="workflow-assignment-draft-section"
      className={`mt-3 bg-surface border border-border rounded-[14px] shadow-sm ${draftPanelCollapsed ? 'py-[10px] px-3' : 'p-[10px]'}`}
    >
      <div className={`flex items-start gap-2 flex-wrap ${draftPanelCollapsed ? 'mb-0' : 'mb-2'}`}>
        ...
        {draftPanelCollapsed ? (
          <div
            ...
            title={formatDraftSummary(uploadDraft, uploadJobInfo)}
          >
        ...
        <button
          type="button"
          className="ghost"
          onClick={() => setDraftPanelCollapsed((v: boolean) => !v)}
        >
      ...
      {draftPanelCollapsed || !uploadDraft ? (
        ...
        <>
          <div className="text-[13px] text-muted grid gap-1">
            ...
            {uploadDraft.requirements_missing && uploadDraft.requirements_missing.length ? (
              <div className="text-[#8a1f1f] font-semibold">
                缺失项：{formatMissingRequirements(uploadDraft.requirements_missing)}
                （补全后才能创建）
              </div>
            ) : (
          ...
          {draftActionStatus && (
            <pre className="mt-[10px] p-[10px_12px] rounded-xl text-[12px] whitespace-pre-wrap overflow-x-auto bg-success-soft text-success">
              {draftActionStatus}
            </pre>
          )}
          ...
          <div className="mt-3 grid gap-3 grid-cols-1">
            <div className="draft-card border border-border rounded-[16px] p-3 bg-white">
              ...
              <div className="grid gap-2">
                <label>1) 学科</label>
                <input
                  value={uploadDraft.requirements?.subject || ''}
                  onChange={(e) => updateDraftRequirement('subject', e.target.value)}
                />
                <label>1) 本节课主题</label>
                <input
                  value={uploadDraft.requirements?.topic || ''}
                  onChange={(e) => updateDraftRequirement('topic', e.target.value)}
                />
                <label>2) 年级</label>
                <input
                  value={uploadDraft.requirements?.grade_level || ''}
                  onChange={(e) => updateDraftRequirement('grade_level', e.target.value)}
                />
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateDraftRequirement('core_concepts', parseCommaList(e.target.value))
                  }
                />
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateDraftRequirement('duration_minutes', Number(e.target.value))
                  }
                >
                  <option value="">未设置</option>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateDraftRequirement('preferences', parseCommaList(e.target.value))
                  }
            ...
            <div className="draft-card border border-border rounded-[16px] p-3 bg-white">
              ...
              <div className="text-muted text-[12px] mb-[10px]">
                题目较多时可先修改关键题，点击"保存草稿"后再创建。
              </div>
              {(uploadDraft.questions || [])
                .slice(0, questionShowCount)
                .map((q: AssignmentQuestion, idx: number) => (
                  <details
                    key={idx}
                    className="border border-border rounded-[14px] py-2 px-[10px] mb-[10px] bg-[#fbfaf7]"
                    open={idx < 1}
                  >
                    <summary>
                    ...
                    <div className="mt-[10px] grid gap-2">
                      ...
                      <div className="grid gap-[10px] grid-cols-1">
                        <div>
                          ...
                          <input
                            value={q.score ?? ''}
                            onChange={(e) =>
                              updateDraftQuestion(idx, { score: Number(e.target.value) || 0 })
                            }
                            placeholder="0"
                          />
                        </div>
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              updateDraftQuestion(idx, { difficulty: e.target.value })
                            }
                          >
                      ...
                      <div className="grid gap-[10px] grid-cols-1">
                        <div>
                          ...
                          <input
                            value={q.kp || ''}
                            onChange={(e) => updateDraftQuestion(idx, { kp: e.target.value })}
                          />
                        </div>
                        <div>
                          <label>标签（逗号分隔）</label>
                          <input
                            ...
                            onChange={(e) =>
                              updateDraftQuestion(idx, { tags: parseCommaList(e.target.value) })
                            }
                          />
                        </div>
                      </div>
                      ...
                      <input
                        value={q.type || ''}
                        onChange={(e) => updateDraftQuestion(idx, { type: e.target.value })}
                      />
                    </div>
                  </details>
                ))}
              {uploadDraft.questions && uploadDraft.questions.length > questionShowCount && (
                <div className="mt-[10px] flex gap-[10px] justify-end flex-wrap">
                  <button
                    type="button"
                    className="border border-border rounded-xl py-[10px] px-[14px] bg-white text-ink cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
                    onClick={() => setQuestionShowCount((n: number) => n + 20)}
                  >
                    加载更多（+20）
                  </button>
                </div>
