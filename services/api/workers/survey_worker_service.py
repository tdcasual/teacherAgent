...
def enqueue_survey_job_inline(job_id: str, *, deps: SurveyWorkerDeps) -> bool:
    enqueued = False
    with deps.job_lock:
...
def scan_pending_survey_jobs_inline(*, deps: SurveyWorkerDeps) -> int:
    ...
    for job_path in deps.job_dir.glob("*/job.json"):
        try:
            ...
            _log.warning("corrupt survey job.json at %s, skipping", job_path, exc_info=True)
            continue
        status = str(data.get("queue_status") or data.get("status") or "")
        job_id = str(data.get("job_id") or "")
        if status in {"queued", "processing", "webhook_received"} and job_id:
            if enqueue_survey_job_inline(job_id, deps=deps):
                count += 1
    return count



def survey_job_worker_loop(*, deps: SurveyWorkerDeps) -> None:
    while not deps.stop_event.is_set():
        ...
        try:
            ...
            _log.debug("operation failed", exc_info=True)
            deps.diag_log("survey.job.failed", {"job_id": job_id, "error": str(exc)[:200]})
            deps.write_job(
                job_id,
                {
                    "status": "failed",
...
def start_survey_worker(*, deps: SurveyWorkerDeps) -> None:
    if deps.rq_enabled():
        return
    ...
    scan_pending_survey_jobs_inline(deps=deps)
    thread = deps.thread_factory(target=lambda: survey_job_worker_loop(deps=deps), daemon=True, name="survey-worker")
    thread.start()
...
def stop_survey_worker(*, deps: SurveyWorkerDeps, timeout_sec: float = 1.5) -> None:
    if deps.rq_enabled():
        return
    deps.stop_event.set()
    deps.job_event.set()
    ...
    if thread is not None:
        try:
            ...
            _log.debug("survey worker thread join failed", exc_info=True)
