...
def build_router(core) -> APIRouter:
    ...
    async def create_skill(req: TeacherSkillCreateRequest):
        try:
    ...
    async def update_skill(skill_id: str, req: TeacherSkillUpdateRequest):
        try:
    ...
    async def import_skill(req: TeacherSkillImportRequest):
        try:
            return await run_in_threadpool(core.import_skill_from_github, github_url=req.github_url)
