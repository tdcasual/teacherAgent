...
export const buildExecutionTimelineEntry = (
  ...
  if (eventType === 'workflow.resolved') {
    const skillId = String(
      payload.requested_skill_id ||
        payload.skill_id_requested ||
        payload.effective_skill_id ||
        payload.skill_id_effective ||
        '',
    ).trim();
    const effective = String(payload.effective_skill_id || payload.skill_id_effective || '').trim();
    const title =
      skillList.find((item) => item.id === effective)?.title || effective || skillId || 'workflow';
  ...
  if (eventType === 'tool.finish') {
    ...
    return {
      type: eventType,
      summary: ok ? `工具完成：${toolName}` : `工具失败：${toolName}`,
      meta: payload,
    };
  }
  if (eventType === 'assistant.done')
    return { type: eventType, summary: '已生成回复', meta: payload };
  ...
  if (eventType === 'job.cancelled')
    return { type: eventType, summary: '任务已取消', meta: payload };
  return null;
};
...
export const resolveWorkflowHint = (
  ...
  if (confidence !== undefined && confidence < 0.56)
    return `已优先按「${title}」工作流处理；如需切换，可显式选择能力。`;
