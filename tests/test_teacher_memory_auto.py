...
class TeacherAutoMemoryTest(unittest.TestCase):
    def test_explicit_preference_creates_auto_intent_proposal(self):
        with TemporaryDirectory() as td:
            ...
            app_mod.get_core().run_agent = lambda *args, **kwargs: {"reply": "好的，我会按这个格式执行。"}  # type: ignore[attr-defined]
            ...
            listed = app_mod.get_core().teacher_memory_list_proposals(teacher_id, status="applied", limit=50)
    ...
    def test_auto_intent_is_deduplicated(self):
        with TemporaryDirectory() as td:
            app_mod = load_app(Path(td))
            app_mod.get_core().run_agent = lambda *args, **kwargs: {"reply": "收到。"}  # type: ignore[attr-defined]

            teacher_id = app_mod.get_core().resolve_teacher_id("teacher")
            text = "请记住：以后默认输出三段式总结，先结论再行动项。"

            rec1 = build_teacher_job("cjob_auto_memory_101", "req_auto_memory_101", teacher_id, text)
            app_mod.get_core().write_chat_job(rec1["job_id"], rec1, overwrite=True)
            app_mod.get_core().process_chat_job(rec1["job_id"])

            rec2 = build_teacher_job("cjob_auto_memory_102", "req_auto_memory_102", teacher_id, text)
            app_mod.get_core().write_chat_job(rec2["job_id"], rec2, overwrite=True)
            app_mod.get_core().process_chat_job(rec2["job_id"])

            listed = app_mod.get_core().teacher_memory_list_proposals(teacher_id, status="applied", limit=100)
            proposals = listed.get("proposals") or []
            auto_items = [p for p in proposals if p.get("source") == "auto_intent"]
    ...
    def test_near_compaction_creates_auto_flush_proposal(self):
        with TemporaryDirectory() as td:
            ...
            app_mod.get_core().run_agent = lambda *args, **kwargs: {"reply": "继续执行。"}  # type: ignore[attr-defined]

            teacher_id = app_mod.get_core().resolve_teacher_id("teacher")
            session_id = "main"
            for i in range(3):
                app_mod.get_core().append_teacher_session_message(teacher_id, session_id, "user", f"旧问题{i}")
                app_mod.get_core().append_teacher_session_message(teacher_id, session_id, "assistant", f"旧回答{i}")
            ...
            app_mod.get_core().write_chat_job(rec["job_id"], rec, overwrite=True)
            app_mod.get_core().process_chat_job(rec["job_id"])
    ...
    def test_repeated_preference_without_keyword_can_auto_infer(self):
        with TemporaryDirectory() as td:
            app_mod = load_app(Path(td))
            app_mod.get_core().run_agent = lambda *args, **kwargs: {"reply": "已按你要求输出。"}  # type: ignore[attr-defined]
