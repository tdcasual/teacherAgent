...
export function useTeacherSessionSidebarModel(params: UseTeacherSessionSidebarModelParams) {
  ...
  const visibleHistorySessions = useMemo(() => {
    ...
    return historySessions.filter((item) => {
      ...
      const matched =
        !q || sid.toLowerCase().includes(q) || title.includes(q) || preview.includes(q);
  ...
  const archiveDialogIsArchived = archiveDialogSessionId
    ? deletedSessionIds.includes(archiveDialogSessionId)
    : false;
