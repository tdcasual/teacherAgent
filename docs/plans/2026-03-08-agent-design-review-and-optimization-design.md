...
- 已把 review queue 终态动作规范化写入 `data/analysis/review_feedback.jsonl`；
- 已提供 `/teacher/analysis/ops` 聚合 `runtime_metrics + review_feedback + replay_compare + ops_summary`；
- teacher workbench 已显示 runtime risk、top review reason 与 top recommendation。
...
- 在线 `/teacher/analysis/ops` 只暴露 replay candidate pair 元数据；
- `scripts/export_analysis_ops_snapshot.py` 会导出候选与 compare command hint；
- 真正的 diff 继续由 `scripts/compare_analysis_runs.py` 在离线显式执行。
