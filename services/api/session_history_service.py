...
def student_history_sessions(student_id: str, limit: int, cursor: int, *, deps: SessionHistoryDeps) -> Dict[str, Any]:
    student_id = (student_id or "").strip()
    if not student_id:
        raise SessionHistoryError(status_code=400, detail="student_id is required")
...
def student_session_view_state(student_id: str, *, deps: SessionHistoryDeps) -> Dict[str, Any]:
    student_id = (student_id or "").strip()
    if not student_id:
        raise SessionHistoryError(status_code=400, detail="student_id is required")
...
def update_student_session_view_state(req: Dict[str, Any], *, deps: SessionHistoryDeps) -> Dict[str, Any]:
...
def student_history_session(
    ...
    *,
    ...
    if not student_id or not session_id:
        raise SessionHistoryError(status_code=400, detail="student_id and session_id are required")
