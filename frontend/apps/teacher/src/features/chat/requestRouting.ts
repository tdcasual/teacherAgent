...
export const decideSkillRouting = ({
  ...
  const explicitSkillResolved =
    ...
    parsedInvocation.effectiveSkillId === parsedInvocation.requestedSkillId;
  const shouldPinEffectiveSkill =
    explicitSkillResolved && Boolean(parsedInvocation.effectiveSkillId);
  const normalizedWarnings = parsedInvocation.warnings.map((warning) => {
    if (
      !skillPinned &&
      explicitSkillRequested &&
      !explicitSkillResolved &&
      warning.startsWith('未识别的能力：$')
    ) {
