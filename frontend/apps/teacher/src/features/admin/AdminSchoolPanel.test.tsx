    ...
    if (url.includes('/auth/admin/students/import')) {
      ...
      return {
        ...
        json: async () => ({
          ...
          created: 1,
          updated: 0,
          items: [{ student_id: 's_zhang', student_name: '张三', class_name: '高二1班', temp_password: 'StuPass12ab', created: true }],
        }),
      }
    }
...
describe('AdminSchoolPanel', () => {
  ...
  it('imports roster CSV without auto-enrolling, then enrolls via roster and enroll-class', async () => {
    ...
    expect(fetchMock.mock.calls.some((call) => String(call[0]).includes('/auth/admin/students/import'))).toBe(true)
    expect(fetchMock.mock.calls.some((call) => String(call[0]).includes('/enroll-class'))).toBe(false)
    ...
    const posted = fetchMock.mock.calls.map((call) => `${String(call[1]?.method || 'GET').toUpperCase()} ${String(call[0])}`)
    expect(posted.some((item) => item.includes('POST') && item.includes('/auth/admin/roster'))).toBe(true)
    expect(posted.some((item) => item.includes('POST') && item.includes('/auth/admin/enrollments/enroll-class'))).toBe(true)
  })
  ...
  it('claims an orphan assignment for the selected teacher and subject', async () => {
    ...
    expect(fetchMock.mock.calls.some((call) => String(call[0]).includes('/auth/admin/assignments/HW_ORPHAN/claim'))).toBe(true)
  })
})
