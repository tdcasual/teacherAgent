  ...
  return (
    <section className="grid gap-3" aria-labelledby="student-home-materials-title">
      <div className="flex items-center justify-between gap-3">
        <h2 id="student-home-materials-title" className="m-0 text-[16px] font-semibold text-ink">准备材料</h2>
        <span className="text-[12px] text-muted">{materials.length} 份</span>
      </div>
      {materials.length ? (
        <ul className="m-0 grid gap-2 p-0 list-none">
          {materials.map((item) => (
            <li key={`${item.label}-${item.url || ''}`} className="rounded-[18px] border border-border bg-[color:color-mix(in_oklab,var(--color-note)_72%,white)] px-4 py-3">
              {item.url ? (
