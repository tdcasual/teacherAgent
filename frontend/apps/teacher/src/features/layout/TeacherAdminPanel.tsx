...
export default function TeacherAdminPanel({
  ...
  return (
    <div
      ref={panelRef}
      className="absolute right-0 top-[calc(100%+8px)] z-40 grid w-[min(344px,calc(100vw-16px))] max-h-[min(80vh,720px)] max-w-[calc(100vw-16px)] gap-3 overflow-y-auto rounded-[18px] border border-[color:color-mix(in_oklab,var(--color-border)_82%,var(--color-surface))] bg-[linear-gradient(180deg,color-mix(in_oklab,var(--color-surface)_98%,var(--color-surface))_0%,color-mix(in_oklab,var(--color-surface-soft)_84%,var(--color-surface))_100%)] p-3 shadow-[0_12px_28px_rgba(15,23,42,0.12)]"
      role="region"
      aria-label="教师管理面板"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="grid gap-1">
          <div className="inline-flex w-fit items-center rounded-full border border-[color:color-mix(in_oklab,var(--color-border)_76%,var(--color-surface))] bg-[color:color-mix(in_oklab,var(--color-panel)_86%,var(--color-surface))] px-2.5 py-1 text-[11px] font-semibold tracking-[0.12em] text-muted">
            工具抽屉
          </div>
