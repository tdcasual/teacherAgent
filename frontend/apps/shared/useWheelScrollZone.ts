...
export function useWheelScrollZone<Z extends string>(config: WheelScrollZoneConfig<Z>): WheelScrollZoneReturn<Z> {
  ...
  const setZone = useCallback((zone: Z) => { zoneRef.current = zone }, [])
    ...
    const onWheel = (event: WheelEvent) => {
      ...
      if (resetWhen) {
        for (const { zone: z, condition } of resetWhen) {
          if (zone === z && condition) { zone = dz; break }
        }
      }
