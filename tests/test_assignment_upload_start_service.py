...
class AssignmentUploadStartServiceTest(unittest.TestCase):
    ...
    def test_start_upload_still_accepts_tex_and_bmp(self):
        with TemporaryDirectory() as td:
            root = Path(td)
            ...
            result = asyncio.run(
                start_assignment_upload(
                    ...
                    class_name="",
                    student_ids="",
                    files=[_FakeUpload("paper.tex"), _FakeUpload("figure.bmp")],
                    answer_files=None,
                    ocr_mode="FREE_OCR",
                    language="zh",
                    deps=deps,
            ...
            self.assertEqual(queued, ["job_fixed_001"])
            job = writes["job_fixed_001"]
            self.assertEqual(job.get("source_files"), ["paper.tex", "figure.bmp"])
