...
const renderTeacherChatHarness = (apiBase = 'http://localhost:8000') =>
  renderHook(() => {
    ...
    const logsRef = React.useRef<StreamLogs>({
      streamStages: [],
      queueHints: [],
      toolRunSnapshots: [],
    });

    const trackedSetPendingStreamStage = React.useCallback<
    ...
    const trackedSetChatQueueHint = React.useCallback<React.Dispatch<React.SetStateAction<string>>>(
      (value) => {
    ...
    const trackedSetPendingToolRuns = React.useCallback<
      React.Dispatch<React.SetStateAction<PendingToolRun[]>>
    >((value) => {
    ...
    const api = useTeacherChatApi({
      ...
      setWheelScrollZone: ((_: WheelScrollZone) =>
        undefined) as UseTeacherChatApiParams['setWheelScrollZone'],
    });
...
describe('useTeacherChatApi stream mapping', () => {
  it('shows a low-noise workflow explanation when the backend resolves a capability', async () => {
    ...
    localStorageMock.setItem(
      'teacherAuthSubject',
      JSON.stringify({ teacher_id: 'teacher-1', teacher_name: 'Teacher 1' }),
    );
  ...
  it('shows a light fallback hint when the backend falls back to the default workflow', async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL): Promise<Response> => {
      ...
      if (url.includes('/skills')) {
        return jsonResponse({
          skills: [{ id: 'teacher-assignment-ops', title: '考试分析', desc: 'ops' }],
        });
      }
      return jsonResponse({ ok: true });
    });
  ...
  it('reflects queue + tool progress and completes assistant reply from SSE events', async () => {
    ...
    const streamBody = [
      ...
      toSseEvent(6, 'tool.finish', {
        tool_name: 'exam.analysis.get',
        tool_call_id: 'call-2',
        ok: false,
        error: 'timeout',
      }),
    ...
    await waitFor(() => {
      expect(
        result.current.messages.some(
          (item) => item.role === 'assistant' && item.content === '第一段第二段',
        ),
      ).toBe(true);
    });
    ...
    expect(
      result.current.executionTimeline[result.current.executionTimeline.length - 1]?.summary,
    ).toBe('任务完成');
    ...
    expect(
      snapshots.some((runs) =>
        runs.some((run) => run.name === 'exam.get' && run.status === 'running'),
      ),
    ).toBe(true);
    expect(
      snapshots.some((runs) =>
    ...
    expect(fetchMock.mock.calls.some(([input]) => toUrl(input).includes('/chat/status'))).toBe(
      false,
    );
  ...
  it('falls back to polling when stream reconnect keeps failing', async () => {
    ...
    expect(
      result.current.messages.some(
        (item) => item.role === 'assistant' && item.content === 'fallback reply',
      ),
    ).toBe(true);
    expect(result.current.pendingStreamStage).toBe('');
    expect(result.current.pendingToolRuns).toEqual([]);
    expect(result.current.chatQueueHint).toBe('');
    expect(result.current.executionTimeline.map((item) => item.summary)).toEqual([
      '排队中（前方 1）',
      '任务完成',
    ]);
    expect(fetchMock.mock.calls.some(([input]) => toUrl(input).includes('/chat/status'))).toBe(
  ...
  it('does not spam tool-state updates for assistant delta-only chunks', async () => {
    ...
    expect(
      result.current.messages.some((item) => item.role === 'assistant' && item.content === 'ABCDE'),
    ).toBe(true);
  });
  ...
  it('resolves terminal status after clean stream EOF to avoid reconnect spin', async () => {
    ...
    expect(
      result.current.messages.some(
        (item) => item.role === 'assistant' && item.content === 'EOF 后完成',
      ),
    ).toBe(true);
  });
  ...
  it('falls back to polling on unsupported stream event version', async () => {
    ...
    expect(
      result.current.messages.some(
        (item) => item.role === 'assistant' && item.content === '版本降级成功',
      ),
    ).toBe(true);
  });
  ...
  it('consumes SSE metadata when envelope omits type and event_id', async () => {
    ...
    const streamBody = [
      toRawSseEvent(
        1,
        'assistant.delta',
        JSON.stringify({ payload: { delta: 'A' }, event_version: 1 }),
      ),
    ...
    expect(
      result.current.messages.some((item) => item.role === 'assistant' && item.content === 'A'),
    ).toBe(true);
    expect(fetchMock.mock.calls.some(([input]) => toUrl(input).includes('/chat/status'))).toBe(
      false,
    );
  ...
  it('ignores id-less events when cursor already advanced', async () => {
    ...
    const secondStreamBody = [toSseEvent(2, 'job.done', {})].join('');
    ...
    expect(
      result.current.messages.some((item) => item.role === 'assistant' && item.content === 'AB'),
    ).toBe(false);
  });
  ...
  it('opens tool confirm from SSE and posts /teacher/tools/confirm', async () => {
    ...
    const fetchMock = vi.fn(
      async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
        ...
        if (url.includes('/chat/status'))
          return jsonResponse({ job_id: 'job-confirm', status: 'processing' });
