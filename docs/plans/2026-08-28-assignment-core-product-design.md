...
| D10 | 沿用既有 Owner 约束：fail-closed；Bearer not Cookie；保留 MCP sidecar（剥 exam tools）；无 git filter-repo；prod backup profile 默认关；本波无 AES-GCM；coverage 84% 地板；M/H TDD。 | 与 `docs/plans/2026-08-26-audit-remediation-design.md` 一致，不回退。 |
