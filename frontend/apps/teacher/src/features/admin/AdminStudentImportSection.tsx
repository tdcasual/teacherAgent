...
export default function AdminStudentImportSection({ apiBase }: { apiBase: string }) {
  ...
  const handleImport = async (event: FormEvent) => {
    ...
    try {
      ...
      setInfo(`已写入 student_auth：新建 ${data.created ?? 0}，更新 ${data.updated ?? 0}。导入不会自动编班，请在下方加任教后整班入学。`)
    } catch (err) {
  ...
  return (
    <form className="grid gap-3 rounded-2xl border border-border p-4" onSubmit={handleImport}>
      ...
      <div className="text-xs text-muted">只创建/更新登录账号，不自动 enroll。表头：student_name,class_name，可选 student_id。</div>
      <div className="grid gap-1">
        <label className="text-xs text-muted" htmlFor={fileId}>名册 CSV</label>
        <input
          id={fileId}
          type="file"
          accept=".csv,text/csv"
          ...
          {items.map((item) => {
            ...
            return (
              <div key={studentId} className="grid gap-1 rounded-lg border border-border px-3 py-2">
                <div className="text-sm">{toText(item.student_name)} · {toText(item.class_name)}</div>
