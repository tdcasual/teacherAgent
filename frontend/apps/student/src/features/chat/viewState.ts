...
export const normalizeSessionViewStatePayload = (raw: unknown): SessionViewStatePayload => {
  ...
  const hiddenRaw: unknown[] = Array.isArray(rawRecord.hidden_ids) ? rawRecord.hidden_ids : [];
  const hiddenIds = Array.from(
    new Set(hiddenRaw.map((item: unknown) => String(item || '').trim()).filter(Boolean)),
  );
...
export const readStudentLocalViewState = (studentId: string): SessionViewStatePayload => {
  ...
  try {
    ...
    if (raw) {
      ...
      if (!parsed.active_session_id) {
        parsed.active_session_id = String(
          safeLocalStorageGetItem(`studentActiveSession:${sid}`) || '',
        ).trim();
      }
  ...
  try {
    ...
    if (Array.isArray(parsed))
      hiddenIds = parsed.map((item) => String(item || '').trim()).filter(Boolean);
  } catch {
    hiddenIds = [];
  }
  return normalizeSessionViewStatePayload({
