...
export default function AdminSchoolPanel() {
  ...
  const refreshList = useCallback(async () => {
    setLoadingList(true);
    try {
      const res = await fetch(`${apiBase}/auth/admin/teacher/list`, {
        headers: adminJsonHeaders(),
      });
      const data = (await res.json()) as {
        ok?: boolean;
        items?: TeacherAuthItem[];
        detail?: string;
      };
      if (!res.ok) {
    ...
    >
      <div className="grid gap-6 max-w-[960px]">
        ...
        <form className="grid gap-3 rounded-2xl border border-border p-4" onSubmit={handleCreate}>
          ...
          <div className="grid gap-1">
            <label className="text-xs text-muted" htmlFor={emailId}>
              邮箱（可选）
            </label>
            <input
              id={emailId}
          ...
          <div className="grid gap-1">
            <label className="text-xs text-muted" htmlFor={teacherIdField}>
              教师 ID（可选）
            </label>
            <input
              id={teacherIdField}
                  ...
                  >
                    <div className="grid gap-0.5 min-w-0">
                      <div className="text-sm font-medium">
                        {toText(item.teacher_name) || teacherId}
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        className="ghost"
                        onClick={() => void handleDisable(teacherId, !disabled)}
                      >
                        {disabled ? '启用' : '禁用'}
                      </button>
                      <button
                        type="button"
                        className="ghost"
                        onClick={() => void handleReset(teacherId)}
                      >
                        重置密码
                      </button>
                    </div>
