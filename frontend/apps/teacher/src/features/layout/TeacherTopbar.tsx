...
export default function TeacherTopbar({
  ...
  const authLayerRefs = useMemo(() => [authPanelRef, authButtonRef] as const, []);
  ...
  return (
    ...
    >
      <div
        className={`flex items-center gap-[10px] flex-wrap ${compactMobile ? 'max-[900px]:gap-2 max-[900px]:flex-nowrap' : ''}`.trim()}
      >
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div
            className={`mobile-topbar-title font-bold text-[16px] tracking-[0.2px] ${compactMobile ? 'max-[900px]:text-[14px] max-[900px]:truncate' : ''}`.trim()}
          >
      ...
      <div
        className={`flex gap-[10px] items-center flex-wrap relative ${compactMobile ? 'max-[900px]:gap-2 max-[900px]:flex-nowrap' : ''}`.trim()}
      >
        <div className={`role-badge teacher ${compactMobile ? 'max-[900px]:hidden' : ''}`.trim()}>
          {isAdmin ? '身份：管理员' : '身份：老师'}
        </div>
        {authed ? (
          <span
            className={`text-xs text-muted ${compactMobile ? 'max-[900px]:hidden' : ''}`.trim()}
          >
            已认证：{authSubjectLabel || '教师'}
          </span>
        ) : null}
        {!compactMobile ? (
          <button
