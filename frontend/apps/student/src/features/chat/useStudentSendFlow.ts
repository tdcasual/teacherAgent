...
export function useStudentSendFlow(params: UseStudentSendFlowParams) {
  ...
  const handleSend = useCallback(
    async (event: FormEvent) => {
      ...
      const syncPendingFromStorage = async ({
        verifyRemote = false,
        ...
        if (verifyRemote) {
          try {
            const statusRes = await fetch(
              `${apiBase}/chat/status?job_id=${encodeURIComponent(parsed.job_id)}`,
            );
      ...
      const lockAcquired = await withStudentSendLock(studentId, async () => {
        ...
        const sessionAssignmentId = String(
          sessions.find((item) => item.session_id === sessionId)?.assignment_id || '',
        ).trim();
        ...
        const contextMessages = [
          ...messages,
          { id: 'temp', role: 'user' as const, content: userText, time: '' },
        ]
          .slice(-40)
        ...
        try {
          ...
          updateMessage(placeholderId, {
            content: `抱歉，请求失败：${toErrorMessage(err)}`,
            time: nowTime(),
          });
          setSending(false);
