...
describe('useTeacherPendingChatJob', () => {
  ...
  it('overlays the pending user and placeholder bubbles for the matching session', () => {
    ...
    expect(result.current.messages.some((msg) => msg.role === 'user' && msg.content === job.user_text)).toBe(true)
    expect(result.current.messages.some((msg) => msg.id === job.placeholder_id && msg.content === '正在生成…')).toBe(
      true,
    )
  })
