  ...
  return (
    <>
      <div className="flex justify-between items-center gap-1.5 max-[900px]:sticky max-[900px]:top-0 max-[900px]:z-1 max-[900px]:bg-white max-[900px]:pb-1.5">
        ...
        <div className="flex items-center gap-1.5 [&_.ghost]:px-[9px] [&_.ghost]:py-1 [&_.ghost]:text-[11px]">
          ...
          <button
            ...
            onClick={() =>
              dispatch({ type: 'SET', field: 'showArchivedSessions', value: !showArchivedSessions })
            }
          >
      ...
      <div className="grid gap-1.5 max-[900px]:sticky max-[900px]:top-[34px] max-[900px]:z-1 max-[900px]:bg-white max-[900px]:pb-1">
        <input
          className="!px-2.5 !py-2 !rounded-[10px] !text-[13px]"
          value={historyQuery}
          onChange={(e) => dispatch({ type: 'SET', field: 'historyQuery', value: e.target.value })}
          placeholder="搜索会话"
          disabled={!verifiedStudent}
        />
      </div>
      {!verifiedStudent && (
        <div className="text-xs text-muted">请先完成姓名验证后查看历史记录。</div>
      )}
      ...
      {verifiedStudent && !historyLoading && visibleSessionCount === 0 && !historyError && (
        <div className="text-xs text-muted">
          {showArchivedSessions ? '暂无归档会话。' : '暂无历史记录。'}
        </div>
      )}
              ...
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    <button
                      type="button"
                      className="session-select w-full border-none bg-transparent pr-7 text-left cursor-pointer block"
                    ...
                    <div className="session-menu-wrap absolute top-1.5 right-1.5">
                      <button
                        ...
                        onKeyDown={(event) =>
                          handleSessionMenuTriggerKeyDown(sid, isMenuOpen, event)
                        }
                      >
                        ⋯
                      </button>
      ...
      {verifiedStudent && (
        <div className="flex gap-2 items-start flex-wrap">
          <button
            ...
            onClick={() => void refreshSessions('more')}
          >
