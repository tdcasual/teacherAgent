...
} from '../features/chat/pendingChatJob';
...
export function useChatPolling({
  ...
  useEffect(() => {
    recentCompletedRepliesRef.current = state.recentCompletedReplies;
  }, [state.recentCompletedReplies, recentCompletedRepliesRef]);
  ...
  // Persist sidebarOpen
  useEffect(() => {
    safeLocalStorageSetItem('studentSidebarOpen', state.sidebarOpen ? 'true' : 'false');
  }, [state.sidebarOpen]);
  ...
  useEffect(() => {
    ...
    if (!sid) {
      ...
      dispatch({ type: 'SET', field: 'pendingChatJob', value: null });
      return;
    }
  ...
  useEffect(() => {
    const sid = verifiedStudent?.student_id?.trim() || '';
    if (!sid) {
      dispatch({ type: 'SET', field: 'recentCompletedReplies', value: [] });
      return;
    }
  ...
  useEffect(() => {
    ...
    const key = `${RECENT_COMPLETION_KEY_PREFIX}${sid}`;
    try {
      if (state.recentCompletedReplies.length)
        safeLocalStorageSetItem(key, JSON.stringify(state.recentCompletedReplies));
      else safeLocalStorageRemoveItem(key);
    } catch {
      /* ignore */
    }
  ...
  useEffect(() => {
    ...
    const sameRecentReplies = (a: RecentCompletedReply[], b: RecentCompletedReply[]) => {
      ...
      for (let i = 0; i < a.length; i++) {
        if (recentCompletionKeyOf(a[i]) !== recentCompletionKeyOf(b[i])) return false;
      }
      return true;
    };
    ...
    const syncFromStorageSnapshot = () => {
      ...
      if (nextSid !== sid) {
        dispatch({ type: 'SET', field: 'verifiedStudent', value: parsedVerified });
        return;
      }
      ...
      updater: (prev) => {
        ...
        if (!hasUserText && pendingChatJob.user_text)
          next.push({
            id: makeId(),
            role: 'user' as const,
            content: pendingChatJob.user_text,
            time: nowTime(),
          });
        if (!hasPlaceholder)
          next.push({
            id: pendingChatJob.placeholder_id,
  ...
  useEffect(() => {
    ...
    if (
      ...
      activeSessionId &&
      pendingChatJob.session_id !== activeSessionId
    )
      return;
      ...
      pollCleanup = startVisibilityAwareBackoffPolling(
        async ({ signal }) => {
          ...
          const res = await fetch(
            `${apiBase}/chat/status?job_id=${encodeURIComponent(pendingChatJob.job_id)}`,
            {
              signal,
            },
          );
          if (res.status === 404) {
            dispatch({
              type: 'UPDATE_MESSAGES',
              updater: (prev) => stripTransientPendingBubbles(prev),
            });
            skipAutoSessionLoadIdRef.current = pendingChatJob.session_id || '';
            dispatch({
              type: 'BATCH',
