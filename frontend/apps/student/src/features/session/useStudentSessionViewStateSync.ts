...
export function useStudentSessionViewStateSync({
  state,
  dispatch,
  setActiveSession,
}: UseStudentSessionViewStateSyncParams) {
  const { apiBase, activeSessionId, sessionTitleMap, deletedSessionIds, localDraftSessionIds } =
    state;
    ...
    const bootstrap = async () => {
      try {
        const res = await fetch(
          `${apiBase}/student/session/view-state?student_id=${encodeURIComponent(sid)}`,
        );
        if (!res.ok) {
  ...
  useEffect(() => {
    ...
    safeLocalStorageSetItem(
      `${STUDENT_SESSION_VIEW_STATE_KEY_PREFIX}${sid}`,
      JSON.stringify(currentViewState),
    );
    safeLocalStorageSetItem(
      `studentSessionTitles:${sid}`,
      JSON.stringify(currentViewState.title_map),
    );
    safeLocalStorageSetItem(
      `studentDeletedSessions:${sid}`,
      JSON.stringify(currentViewState.hidden_ids),
    );
  ...
  useEffect(() => {
    ...
    try {
      safeLocalStorageSetItem(
        `${STUDENT_LOCAL_DRAFT_SESSIONS_KEY_PREFIX}${sid}`,
        JSON.stringify(localDraftSessionIds),
      );
    } catch {
      /* ignore */
    }
